...
def JungliBot_cmd(add_cmd, is_args=False):
    def cmd(func):
        JungliBot = bot.tgbot
        if is_args:
...
def is_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            JungliBot = bot.tgbot
            perms = await JungliBot.get_permissions(event.chat_id, event.sender_id)
...
def is_bot_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            JungliBot = bot.tgbot
            boat = await JungliBot.get_me()
            perms = await JungliBot.get_permissions(event.chat_id, boat)
            if perms.is_admin:
                await func(event)
            else:
