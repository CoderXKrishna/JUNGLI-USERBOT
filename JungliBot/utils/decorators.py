...
def jungli_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                junglireg = "^" + Config.HANDLER
                reg = Config.HANDLER[1]
            elif len(Config.HANDLER) == 1:
                junglireg = "^\\" + Config.HANDLER
                reg = Config.HANDLER
            args["pattern"] = re.compile(junglireg + pattern)
    ...
    # blacklisted chats. 
    # jungliBot will not respond in these chats.
