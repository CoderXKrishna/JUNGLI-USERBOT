...
async def restart(event):
    if HEROKU_APP_NAME and HEROKU_API_KEY:
        ...
        execl(executable, executable, "bash", "JungliBot")


@bot.on(jungli_cmd(pattern="restart$"))
@bot.on(sudo_cmd(pattern="restart$", allow_sudo=True))
async def re(jungli):
    if jungli.fwd_from:
        return
    event = await eor(jungli, "Restarting Dynos ...")
    if HEROKU_API_KEY:
        await restart(event)
    else:
        await event.edit("Please Set Your `HEROKU_API_KEY` to restart Jυɳɠʅι Aɾαɳƙ Bσƚ")


@bot.on(jungli_cmd(pattern="shutdown$"))
@bot.on(sudo_cmd(pattern="shutdown$", allow_sudo=True))
async def down(jungli):
    if jungli.fwd_from:
        return
    event = await eor(jungli, "`Turing Off Heroku Dynos...`")
    await asyncio.sleep(2)
    await event.edit("**[ ⚠️ ]** \n**Jυɳɠʅι Aɾαɳƙ Bσƚ Dynos is now turned off. Manually turn it on to start again.**")
...
@bot.on(jungli_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", outgoing=True))
@bot.on(sudo_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", allow_sudo=True))
async def variable(jungli):
    if jungli.fwd_from:
        return
    if Config.HEROKU_APP_NAME is not None:
        app = Heroku.app(Config.HEROKU_APP_NAME)
    else:
        return await eor(jungli, "`[HEROKU]:" "\nPlease setup your` **HEROKU_APP_NAME**")
    exe = jungli.pattern_match.group(1)
    heroku_var = app.config()
    if exe == "get":
        event = await eor(jungli, "Getting Variable Info...")
        await asyncio.sleep(1.5)
        cap = "Logger me chala jaa bsdk."
        capn = "Saved in LOGGER_ID !!"
        try:
            variable = jungli.pattern_match.group(2).split()[0]
            if variable in ("jungli_DANAV_SESSION", "BOT_TOKEN", "HEROKU_API_KEY"):
                if Config.ABUSE == "ON":
                    await bot.send_file(jungli.chat_id, cjb, caption=cap)
                    await event.delete()
        ...
        event = await eor(jungli, "Setting Heroku Variable...")
        variable = jungli.pattern_match.group(2)
        if not variable:
            return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        value = jungli.pattern_match.group(3)
        if not value:
            variable = variable.split()[0]
            try:
                value = jungli.pattern_match.group(2).split()[1]
            except IndexError:
                return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        await asyncio.sleep(1.5)
        if variable in heroku_var:
        ...
        event = await eor(jungli, "Getting info to delete Variable")
        try:
            variable = jungli.pattern_match.group(2).split()[0]
        except IndexError:
...
@bot.on(jungli_cmd(pattern="usage(?: |$)", outgoing=True))
@bot.on(sudo_cmd(pattern="usage(?: |$)", allow_sudo=True))
async def dyno_usage(jungli):
    if jungli.fwd_from:
        return
    event = await edit_or_reply(jungli, "`Processing...`")
    useragent = (
    ...
    return await event.edit(
        ...
        f"     ★  `{hours}`**h**  `{minutes}`**m**  "
        f"**|**  `{percentage}`**%**"
        f"\n\n**Owner :** {jungli_mention}"
    )


@bot.on(jungli_cmd(pattern="logs$"))
@bot.on(sudo_cmd(pattern="logs$", allow_sudo=True))
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await eor(dyno, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit {jungli_grp} for help.", link_preview=False)
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)
    except BaseException:
        return await dyno.reply(f"Make Sure Your Heroku AppName & API Key are filled correct. Visit {jungli_grp} for help.", link_preview=False)
   # event = await eor(dyno, "Downloading Logs...")
    jungli_data = app.get_log()
    await eor(dyno, jungli_data, deflink=True, linktext=f"**🗒️ Heroku Logs of 💯 lines. 🗒️**\n\n🌟 **Bot Of :**  {jungli_mention}\n\n🚀** Pasted**  ")
