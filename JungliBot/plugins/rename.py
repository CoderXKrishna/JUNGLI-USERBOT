...
@bot.on(jungli_cmd(pattern="rename (.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="rename (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    jungli = await eor(event, 
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            await jungli.edit("Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
            )
        else:
            await eod(jungli, "Error Occurred\n {}".format(input_str))
    else:
        await eod(jungli, f"Syntax `{hl}rename file.name` as reply to a Telegram media")


@bot.on(jungli_cmd(pattern="rnupload (.*)", outgoing=True))
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            ...
            await jungli.edit("Downloaded in {} seconds. Uploaded in {} seconds.".format(
                    ms_one, ms_two
                )
            )
        else:
        ...
        await jungli.edit("Syntax // `{}rnupload file.name` as reply to a Telegram media".format(hl))


@bot.on(jungli_cmd(pattern="rnsupload (.*)", outgoing=True))
        ...
        if os.path.exists(downloaded_file_name):
            ...
            try:
                await bot.send_file(
                    ...
                    downloaded_file_name,
                    thumb=thumb,
                    caption="reuploaded by [JungliBot](https://t.me/Carding_Chronicle)",
                    force_document=False,
                    allow_cache=False,
                    reply_to=event.message.id,
                    attributes=[
            ...
            await eod(jungli, "File Not Found {}".format(input_str))
    else:
        await jungli.edit(
