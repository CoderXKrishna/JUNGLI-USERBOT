...
async def set_not_afk(event):
    ...
    if "#" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        JungliBot = await event.client.send_message(
            event.chat_id,
        ...
        try:
            ...
            await bot.send_message(  # pylint:disable=E0602
                ...
                + f"Ask in {jungli_grp} to get help!",
                reply_to=event.message.id,
                link_preview=False,
                silent=True,
            )
        await asyncio.sleep(5)
        await JungliBot.delete()
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        ...
        msg = await event.reply(message_to_reply, file=junglipic)
...
@bot.on(jungli_cmd(pattern=r"afk (.*)", outgoing=True))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    ...
    junglipic = await event.client.download_media(krakenop)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        USER_AFK = f"yes: {reason} {junglipic}"  # pylint:disable=E0602
        if reason:
            await bot.send_message(
                event.chat_id, f"**I'm going afk🚶** \n\n**Because :** {reason}", file=junglipic
            )
        else:
            await bot.send_message(
                event.chat_id, f"**I am Going afk!**🚶", file=junglipic)
        ...
        try:
            if reason:
                await bot.send_message(
                  Config.LOGGER_ID,
                  f"#AFKTRUE \nAFK mode = **True**\nReason  `{reason}`",file=junglipic
                 )
            else:
                await bot.send_message(
                  Config.LOGGER_ID,
                  f"#AFKTRUE \nAFK mode = **True**",file=junglipic
            )
        except Exception as e:  # pylint:disable=C0103,W0703
...
@bot.on(jungli_cmd(pattern=r"night ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    global USER_night
