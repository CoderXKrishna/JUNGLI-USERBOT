...
JungliBot = Config.STICKER_PACKNAME


@bot.on(jungli_cmd(outgoing=True, pattern="kang"))
...
async def kang(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            jungli = await eor(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        packname = f"{user.username}_JungliBot_{pack}"
        packnick = (
            f"{JungliBot} Vol.{pack}"
            if JungliBot
            else f"@{user.username}'s JungliBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            await jungli.edit("`Preparing a new pack....`")
            async with bot.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
...
@bot.on(jungli_cmd(pattern=r"delst ?(.*)", outgoing=True))
...
@bot.on(jungli_cmd(pattern=r"editst ?(.*)", outgoing=True))
...
async def _(event):
    ...
    if not event.reply_to_msg_id:
        ...
        return
    reply_message = await event.get_reply_message()
    jungli = event.pattern_match.group(1)
    chat = "@Stickers"
    reply_message.sender
    if reply_message.sender.bot:
        await edit_or_reply(event, "`Reply to actual user's message.`")
    ...
    if jungli == "":
        ...
        async with bot.conversation(chat) as conv:
            try:
                ...
                await conv.get_response()
                await asyncio.sleep(2)
                await conv.send_message(f"{jungli}")
                response = await response
            except YouBlockedUserError:
                await event.reply("Please unblock @Stickers and try again")
                return
...
@bot.on(jungli_cmd(pattern="text (.*)"))
...
async def sticklet(event):
    ...
    image_stream.name = "JungliBot.webp"
...
@bot.on(jungli_cmd(pattern="waifu(?: |$)(.*)", outgoing=True))
