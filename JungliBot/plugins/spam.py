...
@bot.on(jungli_cmd(pattern="spam (.*)"))
...
@bot.on(jungli_cmd(pattern="bigspam"))
@bot.on(sudo_cmd(pattern="bigspam", allow_sudo=True))
async def bigspam(jungli):
    if not jungli.text[0].isalpha() and jungli.text[0] not in ("/", "#", "@", "!"):
        jungli_msg = jungli.text
        JungliBot_count = int(jungli_msg[9:13])
        jungli_spam = str(jungli.text[13:])
        for i in range(1, JungliBot_count):
            await jungli.respond(jungli_spam)
        await jungli.delete()
        await jungli.client.send_message(
                lg_id, f"#BIGSPAM \n\nBigspammed  `{jungli_count}`  messages !!"
        )


@bot.on(jungli_cmd("dspam (.*)"))
...
@bot.on(jungli_cmd(pattern="mspam (.*)"))
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what JungliBot iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add_info(
  "Spammers Commands"
).add_warning(
