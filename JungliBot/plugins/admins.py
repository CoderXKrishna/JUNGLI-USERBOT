...
from . import *
from JungliBot.sql.mute_sql import is_muted, mute, unmute
...
@bot.on(jungli_cmd(pattern="setgpic$"))
...
@bot.on(jungli_cmd(pattern="promote(?: |$)(.*)"))
...
async def promote(promt):
    ...
    junglievent = await eor(promt, "`Promoting User...`")
    ...
    try:
        ...
        await junglievent.edit(f"**🔥 Kʀ Dɪʏᴀ Pʀᴏᴍᴏᴛᴇ Cʜʟ ᴀʙ Eᴋ Kᴀᴀᴍ ᴋʀ Cʜᴀᴋɴᴀ Mᴇɪɴ Lᴀᴀᴛᴀ Hᴜ ᴛᴜ DAᴀʀᴜ LAᴀ PAᴡʀʀʏʏ ᴋʀᴇɢᴇ Aᴘᴀɴ")
    except BadRequestError:
        await junglievent.edit(NO_PERM)
        return
...
@bot.on(jungli_cmd(pattern="promote1(?: |$)(.*)"))
...
@bot.on(jungli_cmd(pattern="promote2(?: |$)(.*)"))
...
@bot.on(jungli_cmd(pattern="promote3(?: |$)(.*)"))
...
@bot.on(jungli_cmd(pattern="demote(?: |$)(.*)"))
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    junglievent = await eor(dmod, "`Demoting User...`")
    rank = "ǟɖʍɨռ"
    ...
    try:
        ...
        await junglievent.edit(NO_PERM)
        return
    await junglievent.edit(f"**😪 Bʜᴀɪ/Bᴇʜᴇɴ ᴛᴇʀɪ Aᴜᴋᴀᴀᴛ Nᴀ Hᴀɪ Iᴅʜʀ Aᴅᴍɪɴ BAɴɴᴇ Kɪ`")
...
@bot.on(jungli_cmd(pattern=r"mute ?(.*)"))
@bot.on(sudo_cmd(pattern=r"mute ?(.*)", allow_sudo=True))
async def muth(jungli):
    if jungli.is_private:
        await eor(jungli, "**Enough of your bullshit  !!**")
        await sleep(2)
        await jungli.get_reply_message()
        replied_user = await jungli.client(GetFullUserRequest(jungli.chat_id))
        if is_muted(jungli.chat_id, jungli.chat_id):
            return await jungli.edit(
                "Nigga is already muted here 🥴"
            )
        if jungli.chat_id == official_sameer:
            return await eod(jungli, "Nashe me hai kya lawde 🥴")
        try:
            mute(jungli.chat_id, jungli.chat_id)
        except Exception as e:
            await eor(jungli, f"**Error **\n`{str(e)}`")
        else:
            await eor(jungli, "**Chup Reh Lawde 🥴\n`**｀-´)⊃━☆ﾟ.*･｡ﾟ **`")
    else:
        junglievent = await eor(jungli, "`Muting...`")
        input_str = jungli.pattern_match.group(1)
        chat = await jungli.get_chat()
        if jungli.reply_to_msg_id:
            userid = (await jungli.get_reply_message()).sender_id
            name = (await jungli.client.get_entity(userid)).first_name
        elif input_str:
            if input_str.isdigit():
                try:
                    ...
                    return await junglievent.edit(str(ve))
            else:
                userid = (await jungli.client.get_entity(input_str)).id
                name = (await jungli.client.get_entity(userid)).first_name
        else:
            return await eod(junglievent, "I Need a user to mute!!", 5)
        if userid == official_sameer:
            return await eod(junglievent, "Nashe me hai kya lawde", 5)
        if str(userid) in DEVLIST:
            return await eod(junglievent, "**Error Muting God**", 7)
        try:
            await jungli.client.edit_permissions(
                chat.id,
                userid,
                until_date=None,
                send_messages=False,
            ...
            await eor(junglievent, f"`{str(be)}`")
        await jungli.client.send_message(
            lg_id,
            "#MUTE\n"
...
@bot.on(jungli_cmd(pattern=r"unmute ?(.*)"))
...
async def nomuth(evn):
    if evn.is_private:
        ...
        junglievent = await eor(evn, "`Unmuting...`")
        ...
        if evn.reply_to_msg_id:
            ...
            return await eod(junglievent, "I need a user to unmute!!", 3)
        try:
...
@bot.on(jungli_cmd(pattern="ban(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="ban(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def ban(bon):
    if bon.fwd_from:
        return
    junglievent = await eor(bon, "`Banning Nigga...`")
    ...
    if not user:
        return await junglievent.edit("`Reply to a user or give username!!`")
    if str(user.id) in DEVLIST:
        return await junglievent.edit("**Say again? Ban my creator??**")
    try:
    ...
    try:
        ...
        await junglievent.edit(f"**Banned  [{user.first_name}](tg://user?id={user.id})  in** `[{bon.chat.title}]` !!\n\nMessage Nuking : **False**")
        return
    if reason:
        await junglievent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]` !!\n**Reason :** `{reason}`")
    else:
        await junglievent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]`!!")
...
@bot.on(jungli_cmd(pattern="unban(?: |$)(.*)"))
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    junglievent = await eor(unbon, "`Unbanning...`")
    ...
    try:
        ...
        await junglievent.edit(f"[{user.first_name}](tg://user?id={user.id}) **Is Now Unbanned in**  `{unbon.chat.title}` !!")
        ...
        await junglievent.edit("Invalid UserId!! Please Recheck it!!")


@bot.on(jungli_cmd(pattern="pin($| (.*))"))
...
@bot.on(jungli_cmd(pattern="kick(?: |$)(.*)"))
...
async def kick(usr):
    ...
    junglievent = await eor(usr, "`Kicking...`")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await junglievent.edit(NO_PERM + f"\n`{str(e)}`")
        return
    if reason:
        await junglievent.edit(
            f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`\nReason: `{reason}`"
        )
    else:
        await junglievent.edit(f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`")
...
@bot.on(jungli_cmd(pattern=f"zombies ?(.*)"))
...
@bot.on(jungli_cmd(pattern="undlt$"))
