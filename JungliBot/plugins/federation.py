...
@bot.on(jungli_cmd(pattern="newfed ?(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="newfed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    jungli_input = event.pattern_match.group(1)
    chat = "@MissRose_Bot"
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            await event.client.send_message(chat, f"/newfed {jungli_input}")
...
@bot.on(jungli_cmd(pattern="renamefed ?(.*)"))
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              ...
              await event.client.send_message(chat, f"/renamefed {jungli_input}")
              response = await response 
          except YouBlockedUserError: 
...
@bot.on(jungli_cmd(pattern="fstat ?(.*)"))
@bot.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    jungli = await eor(event, "`Collecting fstat....`")
    thumb = jungli_logo
    ...
    if lavde == "":
        ...
        async with bot.conversation(fbot) as conv:
            try:
                ...
                await jungli.edit("`Please Unblock` @MissRose_Bot")


@bot.on(jungli_cmd(pattern="fedinfo ?(.*)"))
@bot.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    jungli = await eor(event, "`Fetching fed info.... please wait`")
    lavde = event.pattern_match.group(1)
    async with bot.conversation(fbot) as conv:
        try:
            ...
            await jungli.edit(massive.text + "\n\n**ʟɛɢɛռɖaʀʏ_ᴀғ_ɦɛʟʟɮօt**")
        except YouBlockedUserError:
            await jungli.edit("`Please Unblock` @MissRose_Bot")
...
from JungliBot.utils import jungli_cmd, edit_or_reply, sudo_cmd
...
@bot.on(jungli_cmd("superfban ?(.*)"))
@bot.on(sudo_cmd("superfban ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"Starting superfban By [{DEFAULTUSER}](tg://user?id={JungliBot}) 😈")
    fedList = []
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
    ...
    if not len(fedList):
        ...
        if len(fedList) == 0:
            await event.edit("Something went wrong.")
            return
    await event.edit(f"Fbaning in {len(fedList)} feds by [{DEFAULTUSER}](tg://user?id={JungliBot}) 😈")
    try:
    ...
    for fed in fedList:
        ...
        await asyncio.sleep(3)
        await event.client.send_message(chat, f"/fban {FBAN} {REASON} \n\n#SUPERFBAN_BY_JungliBot")
        await asyncio.sleep(3)
    await event.edit(f"SuperFBan Completed. Affected {len(fedList) - exCount} feds by [{DEFAULTUSER}](tg://user?id={JungliBot}) 😈")
