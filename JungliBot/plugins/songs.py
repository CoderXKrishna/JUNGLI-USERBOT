...
@bot.on(jungli_cmd(pattern="lyrics(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="lyrics(?: |$)(.*)", allow_sudo=True))
async def nope(kraken):
    jungli = kraken.pattern_match.group(1)
    await eor(kraken, f"Searching lyrics for  `{jungli}` ...")
    if not jungli:
        if kraken.is_reply:
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(jungli))}")
...
@bot.on(jungli_cmd(pattern="song(?: |$)(.*)", outgoing=True))
...
@bot.on(jungli_cmd(pattern="vsong(?: |$)(.*)", outgoing=True))
