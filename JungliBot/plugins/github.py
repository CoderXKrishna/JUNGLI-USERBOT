...
@bot.on(jungli_cmd(pattern=r"commit"))
...
async def download(event):
    ...
    if Config.GIT_REPO_NAME is None:
        ...
        return
    JungliBot = await eor(event, "Processing ...")
    ...
    try:
        ...
        await eod(JungliBot, str(e))
    else:
        end = datetime.now()
        ms = (end - start).seconds
        await event.delete()
        await JungliBot.edit(
...
async def git_commit(file_name, mikasabot):
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await JungliBot.edit("`File Already Exists`")
            create_file = False
    file_name = "JungliBot/plugins/" + file_name
    if create_file == True:
        ...
        try:
            ...
            await eod(JungliBot, "Cannot Upload Plugin")
    else:
        return await eod(JungliBot, "`Committed Suicide`")


@bot.on(jungli_cmd(pattern="github (.*)", outgoing=True))
