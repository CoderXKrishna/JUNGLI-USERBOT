...
@bot.on(jungli_cmd(pattern="exec(?: |$|\n)(.*)", command="exec"))
...
async def _(event):
    ...
    junglievent = await eor(event, "`Executing.....`")
    process = await asyncio.create_subprocess_sjungli(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    jungliuser = await event.client.get_me()
    if jungliuser.username:
        curruser = jungliuser.username
    else:
        curruser = "JungliBot"
    uid = os.geteuid()
    if uid == 0:
...
@bot.on(jungli_cmd(pattern="eval(?: |$|\n)(.*)", command="eval"))
...
async def _(event):
    ...
    junglievent = await eor(event, "`Running ...`")
    ...
    if exc:
        ...
        evaluation = "Success"
    final_output = f"•  Eval : \n`{cmd}` \n\n•  Result : \n`{evaluation}` \n"
    await eor(junglievent, "**Eval Command Executed. Check out LOGGER for result.**")
    await event.client.send_message(
        lg_id,
...
@bot.on(jungli_cmd(pattern="bash ?(.*)", outgoing=True))
...
CmdHelp("evaluators").add_command(
  ...
  "exec", "<command>", "Execute a Terminal command on JungliBot server and shows details"
).add_command(
