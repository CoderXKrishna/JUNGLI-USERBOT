...
@bot.on(jungli_cmd(pattern=r"logo ?(.*)"))
@bot.on(sudo_cmd(pattern=r"logo ?(.*)", allow_sudo=True))
async def lg1(junglievent):
    event = await eor(junglievent, "`Processing.....`")
    fnt = await get_font_file(junglievent.client, "@HELL_FRONTS")
    if junglievent.reply_to_msg_id:
        rply = await junglievent.get_reply_message()
    ...
    text = junglievent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    file_name = "JungliBot.png"
    img.save(file_name, "png")
    await bot.send_file(
        junglievent.chat_id,
        file_name,
        caption=f"**Made By :** {jungli_mention}",
    )
    await event.delete()
    try:
        os.remove(file_name)
