...
async def all_groups_id(jungli):
    jungligroups = []
    async for dialog in jungli.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            jungligroups.append(entity.id)
    return jungligroups


@bot.on(jungli_cmd(pattern="frwd$"))
...
@bot.on(jungli_cmd(pattern="resend$"))
...
@bot.on(jungli_cmd(pattern=r"fpost (.*)"))
