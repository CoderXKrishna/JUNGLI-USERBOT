...
import asyncio
from JungliBot.sql.gban_sql import is_gbanned, gbaner, ungbaner, all_gbanned
from JungliBot.sql import gmute_sql as gsql
from . import *


@bot.on(jungli_cmd(pattern=r"gban ?(.*)"))
@bot.on(sudo_cmd(pattern=r"gban ?(.*)", allow_sudo=True))
async def _(event):
    jungli = await eor(event, "`Gbanning...`")
    reason = ""
    if event.reply_to_msg_id:
        ...
        return await eod(jungli, "**To gban a user i need a userid or reply to his/her message!!**")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == official_Krishna:
        return await eod(jungli, "🥴 **Nashe me hai kya lawde ‽**")
    if str(userid) in DEVLIST:
        return await eod(jungli, "😑 **GBan my creator ?¿ Really‽**")
    ...
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
                ...
                pass
    gbaner(userid)
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **beta majdur ko khodna 😪 aur** {jungli_mention} **ko chodna... Kabhi sikhana nhi!! 😏**\n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **Is now GBanned by** {jungli_mention} **in**  `{chats}`  **Chats!! 😏**\n\n**📍 Also Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**🔰 Reason :**  `{reason}`"
    if Config.ABUSE == "ON":
        await bot.send_file(event.chat_id, cjb, caption=gmsg)
    else:
        await jungli.edit(ogmsg)


@bot.on(jungli_cmd(pattern=r"ungban ?(.*)"))
@bot.on(sudo_cmd(pattern=r"ungban ?(.*)", allow_sudo=True))
async def _(event):
    jungli = await eor(event, "`Ungban in progress...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        ...
        return await eod(jungli, "`Reply to a user or give their userid... `")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eod(jungli, "`User is not gbanned.`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
...
@bot.on(jungli_cmd(pattern="listgban$"))
...
@bot.on(jungli_cmd(pattern=r"gkick ?(.*)"))
@bot.on(sudo_cmd(pattern=r"gkick ?(.*)", allow_sudo=True))
async def gkick(event):
    jungli = await eor(event, "`Kicking globally...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        userid = await get_user_id(event.pattern_match.group(1))
        ...
        return await eod(jungli, "`Reply to some msg or add their id.`")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == official_Krishna:
        return await eod(jungli, "**🥴 Nashe me hai kya lawde!!**")
    if str(userid) in DEVLIST:
        return await eod(jungli, "**😪 I'm not going to gkick my developer!!**")
    ...
    if Config.ABUSE == "ON":
        ...
        await jungli.edit(gkmsg)


@bot.on(jungli_cmd(pattern=r"gmute ?(\d+)?"))
...
@bot.on(jungli_cmd(outgoing=True, pattern=r"ungmute ?(\d+)?"))
...
# gpromote by Krishna op

# for JungliBot
# ONLY for JungliBot
# EDITED BY - @itsz_krish_babess (Krishna )
# KANGERS STAY AWAY
                                 ...
                                 ChatAdminRights,
                                   ChatBannedRights,
                                     MessageEntityMentionName,
                                       MessageMediaPhoto)
from JungliBot.utils import register, errors_handler
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("Error... Please report at @jungli_USERBOT", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@borg.on(jungli_cmd(pattern="gpromote ?(.*)"))
async def gben(userbot):
    mb = jungli = userbot
    i = 0
    sender = await mb.get_sender()
    me = await userbot.client.get_me()
    await jungli.edit("`promoting...`")
    ...
    try:
        ...
        pass
    if me == user:
       k = await jungli.edit("U want to promote urself 😑😑 waao..")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    except:
        return await jungli.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        ...
        for x in telchanel:
          try:
             ...
             i += 1
             await jungli.edit(f"**Promoted in Chats **: `{i}`")
          except:
             pass
    else:
        await jungli.edit(f"**Reply to a user you dumbo !!**")
    return await jungli.edit(
        f"**Globally promoted [{user.first_name}](tg://user?id={user.id})\n On Chats😏 : {i} **"
    )
@borg.on(jungli_cmd(pattern="gpromote1 ?(.*)"))
async def gben(userbot):
    mb = jungli = userbot
    i = 0
    sender = await mb.get_sender()
...
@borg.on(jungli_cmd(pattern="gpromote2 ?(.*)"))
async def gben(userbot):
    mb = jungli = userbot
    i = 0
    sender = await mb.get_sender()
...
@borg.on(jungli_cmd(pattern="gpromote3 ?(.*)"))
async def gben(userbot):
    mb = jungli = userbot
    i = 0
    sender = await mb.get_sender()
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       k = await jungli.edit("U want to demote urself 😑😑 waao..")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await userbot.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await jungli.edit(f"**Demoted in Chats **: `{i}`")
          except:
             pass
    else:
        await jungli.edit(f"**Reply to a user you dumbo !!**")
