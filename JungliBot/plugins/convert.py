...
@bot.on(jungli_cmd(pattern="stoi$"))
@bot.on(sudo_cmd(pattern="stoi$", allow_sudo=True))
async def _(jungli):
    if jungli.fwd_from:
        return
    reply_to_id = jungli.message.id
    if jungli.reply_to_msg_id:
        reply_to_id = jungli.reply_to_msg_id
    event = await eor(jungli, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await jungli.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await jungli.client.send_file(
                event.chat_id,
...
@bot.on(jungli_cmd(pattern="itos$"))
...
@bot.on(jungli_cmd(pattern="ftoi$"))
...
@bot.on(jungli_cmd(pattern="gif$"))
@bot.on(sudo_cmd(pattern="gif$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    junglireply = await event.get_reply_message()
    if not junglireply or not junglireply.media or not junglireply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if junglireply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    junglievent = await edit_or_reply(event, "`Converting to gif ...`")
    async with bot.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await bot.send_file(chat, junglireply.media)
            response = await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await junglievent.edit("`This file is not supported`")
            jungliresponse = response if response.media else await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            junglifile = Path(await event.client.download_media(jungliresponse, "./temp/"))
            jungligif = Path(await unzip(junglifile))
            ...
            for files in (jungligif, junglifile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await junglievent.edit("Unblock @tgstogifbot")
            return


@bot.on(jungli_cmd(pattern="nfc ?(.*)"))
