...
@bot.on(jungli_cmd(pattern="stats$"))
...
async def stats(
    ...
    if event.fwd_from:
        return
    jungli = await edit_or_reply(event, "`Collecting stats...`")
    ...
    response += (
        f"📌 𝙵𝚁𝙾𝙼 𝚃𝙷𝙴 𝙳𝙰𝚃𝙰𝙱𝙰𝚂𝙴 𝙾𝙵 :-\n[👑 𝙻𝙴𝙶𝙴𝙽𝙳𝚁𝚈 𝙰𝙵 Jυɳɠʅι Aɾαɳƙ Bσƚ 👑](t.me/Carding_Chronicle)\n\n" 
    )
    await jungli.edit(response)
