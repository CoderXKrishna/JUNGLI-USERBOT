...
from JungliBot.helpers import runner
from . import *
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit(
        "✅ Successfully updated Jυɳɠʅι Aɾαɳƙ Bσƚ!\n\nBot is restarting please wait for a minute."
    )
    args = [sys.executable, "-m", "JungliBot"]
    os.execle(sys.executable, *args, os.environ)
    return


@bot.on(jungli_cmd(outgoing=True, pattern=r"update(| now)$"))
...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n**😎 Jυɳɠʅι Aɾαɳƙ Bσƚ is UP-TO-DATE.**"
            f"\n\n**Version :**  {jungli_ver}"
    ...
    if force_update:
        await event.edit(
            "`Force-Updating Jυɳɠʅι Aɾαɳƙ Bσƚ. Please wait...`"
        )
    if conf == "now":
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your Jυɳɠʅι Aɾαɳƙ Bσƚ Is UpToDate**\n\n**Version :**  __{jungli_ver}__\n**Oɯɳҽɾ :**  {jungli_mention}")
    else:
        await event.edit("**Please set up**  `HEROKU_API_KEY`  **from heroku to update!**")
    return


@bot.on(jungli_cmd(outgoing=True, pattern=r"update build$"))
...
async def upstream(event):
    ...
    off_repo = "https://github.com/jungli-FIGHTERS/MISAKA-USERBOT"
    os.chdir("/app")
    git_jungli = f"rm -rf .git"
    try:
        await runner.runcmd(git_jungli)
    except BaseException:
        pass
    try:
        txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit(f"**Jυɳɠʅι Aɾαɳƙ Bσƚ Docker Build In Progress... Type** `{hl}ping`  **after 5 mins to check if Bot is working!**")
