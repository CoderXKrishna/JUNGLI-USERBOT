...
@bot.on(jungli_cmd(pattern="scan ?(.*)"))
...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    junglievent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(junglievent, "`Please unblock `@DrWebBot `and try again`")
            return
        if response.text.startswith("Forward"):
            await eod(junglievent,
...
@bot.on(jungli_cmd(pattern=r"decode$", outgoing=True))
...
@bot.on(jungli_cmd(pattern="barcode ?(.*)"))
@bot.on(sudo_cmd(pattern="barcode ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    junglievent = await eor(event, "...")
    ...
    try:
        ...
        await junglievent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await junglievent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await junglievent.delete()


@bot.on(jungli_cmd(pattern=r"makeqr(?: |$)([\s\S]*)", outgoing=True))
...
async def _(event):
    ...
    start = datetime.datetime.now()
    input_str = event.pattern_match.group(1)
    jungli = await eor(event, "Processing...")
    ...
    if len(input_sgra) == 3:
        ...
        await jungli.edit(str(a))
    else:
        await eod(jungli, f"SYNTAX: {hl}calendar YYYY.MM.DD")
    end = datetime.datetime.now()
    (end - start).seconds


@bot.on(jungli_cmd(pattern="currency (.*)"))
...
@bot.on(jungli_cmd(pattern="currencies$"))
...
@bot.on(jungli_cmd(pattern="ifsc (.*)"))
...
@bot.on(jungli_cmd(pattern="color (.*)"))
...
@bot.on(jungli_cmd(pattern="xkcd ?(.*)"))
@bot.on(sudo_cmd(pattern="xkcd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    junglievent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await junglievent.edit(output_str, link_preview=True)
    else:
        await eod(junglievent, "xkcd n.{} not found!".format(xkcd_id))

@bot.on(jungli_cmd(pattern="dns (.*)", outgoing=True))
...
@bot.on(jungli_cmd(pattern="url (.*)", outgoing=True))
...
@bot.on(jungli_cmd(pattern="unshort (.*)", outgoing=True))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for JungliBot", ".currency 10 usd inr"
).add_command(
