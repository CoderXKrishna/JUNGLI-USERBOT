...
async def _(event):
    ...
    if tgbotusername is not None or ustad_input == "text":
        results = await event.client.inline_query(tgbotusername, "@jungli_SPAM_BOT")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of JungliBot\n\n"
        )
        ustadcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"All commands of the JungliBot are [here]({url})"
