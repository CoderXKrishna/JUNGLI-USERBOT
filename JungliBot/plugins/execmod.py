...
@bot.on(jungli_cmd(pattern="pips(?: |$)(.*)", outgoing=True))
...
@bot.on(jungli_cmd(pattern="suicide$"))
...
@bot.on(jungli_cmd(pattern="date$"))
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[Jυɳɠʅι Aɾαɳƙ Bσƚ's](tg://need_update_for_some_feature/) Environment Module:**\n\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await bot.send_file(
...
@bot.on(jungli_cmd(pattern="speed$"))
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[Jυɳɠʅι Aɾαɳƙ Bσƚ's](tg://need_update_for_some_feature/) , Server Speed Calculated:**\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await bot.send_file(
