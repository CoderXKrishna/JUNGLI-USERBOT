...
@bot.on(jungli_cmd(pattern="picgen"))
...
async def _(event):
    ...
    captin = f"Fake Image By {jungli_mention}"
    fole = "HELLBOT.jpg"
    await bot.send_file(event.chat_id, fole, caption=captin)
    await event.delete()
    os.system("rm /root/JungliBot/HELLBOT.jpg ")


@bot.on(jungli_cmd(pattern="fake ?(.*)"))
...
@bot.on(jungli_cmd(pattern="gbam$"))
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1432756163:
            await reply_message.reply(
                "`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 69$ to my master__ [Krishna](https://t.me/itsz_krish_babess) __to release your account__😏"
            )
        else:
            jnl = (
                "`Warning!! `"
